...
<script type="text/javascript">
	...
	function load_more() {
		...
		var segment_3 = '<?=$this->uri->segment(4)?>';
		var url = '';
		var data = {
			'page_number': page
		};
		if (segment_1 == 'categoryreport') {
			data['slug'] = segment_2;
			url = _BASE_URL + 'public/post_categories/more_reports';
		} else if (segment_1 == 'tag') {
			data['tag'] = segment_2;
			url = _BASE_URL + 'public/post_tags/more_reports';
		} else if (segment_1 == 'archives') {
			data['year'] = segment_2;
			data['month'] = segment_3;
			url = _BASE_URL + 'public/archives/more_reports';
		}
		if ( page <= total_page ) {
			$.post( url, data, function( response ) {
				...
				for (var z in rows) {
					...
					html += '<h4><a href="' + _BASE_URL + 'read/reports/' + result.id + '/' + result.post_slug + '">' + result.post_title + '</a></h4>';
					...
					html += '<p>';
					html += '<a href="' + _BASE_URL + 'read/reports/' +result.id + '/' + result.post_slug + '" class="btn btn-primary btn-sm" role="button">Selengkapnya <i class="fa fa-angle-double-right" aria-hidden="true"></i></a>';
					html += '</p>';
					html += '</div>';
					html += '</div>';
					html += '</div>';
...
<div class="col-xs-12 col-md-8">
		...
		<div class="col-md-4">
			<div class="thumbnail no-border">
				<div class="caption">
					...
					<p>
						<a href="<?=site_url('read/reports/'.$row->id.'/'.$row->post_slug)?>" class="btn btn-primary btn-sm" role="button">Selengkapnya <i class="fa fa-angle-double-right" aria-hidden="true"></i></a>
					</p>
				</div>
			</div>
		</div>
	...
	<?php $idx++; } ?>
	<button class="btn btn-success btn-sm btn-block load-more" onclick="load_more()">MORE REPORT</button>
</div>
