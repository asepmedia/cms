...
class M_orders extends CI_Model {
	...
	public function get_order_lists($where){
		$this->db->where('date', $where);
		return $this->db->get('orders');
	}
	public function get_order_lists_all(){
		return $this->db->get('orders');
	}	
	/**
	 ...
	 * @return Query
	 */
	public function get_where2($id, $limit = 0, $offset = 0, $sort_field = '', $sort_type = 'ASC') {
		$this->db->select('
			x1.id
			, x1.products
			, x1.status
			...
			, x1.date
		');
		$this->db->join('users x2', 'x1.customerid = x2.id', 'LEFT');
		$this->db->join('customers x3', 'x1.join_tb = x3.join_tb', 'LEFT');
		if ($sort_field != '') {
		...
		return $this->db->get(self::$table.' x1', $id);
	}	
	public function get_where($keyword, $limit = 0, $offset = 0, $sort_field = '', $sort_type = 'DESC') {
		...
		$this->db->join('customers x3', 'x1.join_tb = x3.join_tb', 'LEFT');
		$this->db->group_start();
		$this->db->like('x1.products', $keyword);
		$this->db->or_like('x2.user_full_name', $keyword);
		$this->db->or_like('x1.date', $keyword);
		$this->db->group_end();
		if ($sort_field != '') {
			$this->db->order_by($sort_field, $sort_type);
		}
	...
	public function total_rows($keyword) {
		return $this->db
			->join('users x2', 'x1.customerid = x2.id', 'LEFT')
			->group_start()
			->like('x1.products', $keyword)
			->or_like('x2.user_full_name', $keyword)
			->or_like('LEFT(x1.date, 10)', $keyword)
			->group_end()			
			->count_all_results('orders x1');
	}
	...
	public function feed() {
		return $this->db
			->select('id, products, quantity, LEFT(date, 10) AS date')
	...
	public function get_related_posts($post_categories = '', $id) {
		...
		$this->db->where('post_type', 'katalog');
		$this->db->where('is_deleted', 'false');
		$this->db->where('id !=', $id);
		if (! $this->auth->is_logged_in()) {
			$this->db->where('post_visibility', 'public');
	...
	public function get_post_by_tag($tag, $limit= 0) {
		$this->db->select('x1.id, x1.post_title, x2.user_full_name AS post_author, x1.post_content, LEFT(x1.created_at, 10) AS created_at, x1.post_image, x1.post_slug, x1.post_counter');
		$this->db->join('users x2', 'x1.post_author = x2.id', 'LEFT');
		$this->db->where('x1.post_type', 'katalog');
		$this->db->where('x1.is_deleted', 'false');
		$this->db->where('x1.post_status', 'publish');
