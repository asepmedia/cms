...
class Post_tags extends Public_Controller {
	...
	public function reports() {
		...
		if (alpha_dash($tag)) {
			$this->vars['title'] = '#' . ucwords(str_replace('-', ' ', $tag));
			$this->vars['page_title'] = ucfirst(str_replace('-', ' ', $tag));
			$total_rows = $this->m_reports->more_posts_by_tag($tag, 0)->num_rows();
			$this->vars['total_rows'] = $total_rows;
			$this->vars['total_page'] = ceil($total_rows / 6);
			$this->vars['query'] = $this->m_reports->more_posts_by_tag($tag, -1);
	...
	public function katalog() {
		...
		if (alpha_dash($tag)) {
			$this->vars['title'] = '#' . ucwords(str_replace('-', ' ', $tag));
			$this->vars['page_title'] = ucfirst(str_replace('-', ' ', $tag));
			$total_rows = $this->m_katalog->more_posts_by_tag($tag, 0)->num_rows();
			$this->vars['total_rows'] = $total_rows;
			$this->vars['total_page'] = ceil($total_rows / 6);
			$this->vars['query'] = $this->m_katalog->more_posts_by_tag($tag, -1);
	...
	public function more_katalog() {
		...
		$response = [];
		if (alpha_dash($tag)) {
			$query = $this->m_katalog->more_posts_by_tag($tag, $offset);
			$total_rows = $this->m_katalog->more_posts_by_tag($tag, 0)->num_rows();
			$rows = [];
			foreach($query->result() as $row) {
				$rows[] = $row;
