	...
	public function process() {
		...
		if ($this->validation()) {
			...
			if (! $is_exist) {
				...
				$config['protocol'] = $this->session->userdata('mail_server_protocol');
				$config['smtp_host'] = $this->session->userdata('mail_server_hostname');
				$config['smtp_port'] = intval($this->session->userdata('mail_server_port'));
				$config['smtp_user'] = $this->session->userdata('mail_server_username');
				$config['smtp_pass'] = $this->session->userdata('mail_server_password');
				...
				if ($this->email->send()) {
					...
					$response['type'] = 'warning';
					$response['message'] = 'Tautan untuk mengubah kata sandi tidak terkirim. Silahkan kirim email ke ikhwanulzakaria91@gmail.com';
				}
			}
		} else {
			$response['type'] = 'error';
